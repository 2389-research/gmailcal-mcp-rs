...
mod gmail_parsing_tests {
    ...
    fn test_email_json_structure() {
        ...
        assert_eq!(
            multipart_attachment["payload"]["mimeType"],
            "multipart/mixed"
        );
        ...
        assert_eq!(
            attachment_part["body"]["attachmentId"],
            "attachment_id_attachment_id"
        );
    }
    ...
    fn test_special_characters_encoding() {
        ...
        let encoded_body = email_with_emoji["payload"]["body"]["data"]
            .as_str()
            .unwrap();
        assert!(!encoded_body.is_empty());
    ...
    fn test_malformed_email_structure() {
        ...
        let from_headers: Vec<_> = headers.iter().filter(|h| h["name"] == "From").collect();
        assert!(from_headers.is_empty());

        let subject_headers: Vec<_> = headers.iter().filter(|h| h["name"] == "Subject").collect();

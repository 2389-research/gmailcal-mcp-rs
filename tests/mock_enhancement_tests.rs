    ...
    async fn call(&self, method_name: &str) -> GmailResult<String> {
        ...
        if self.error_rate > 0.0 {
            ...
            if random_val < self.error_rate {
                return Err(GmailApiError::NetworkError("Simulated network error".to_string()));
            }
        }
...
mod mock_enhancement_tests {
    ...
    async fn test_partial_error_rate() {
        ...
        assert!(error_count >= iterations * 4 / 10, "Error count too low: {}", error_count);
        assert!(error_count <= iterations * 6 / 10, "Error count too high: {}", error_count);
    }

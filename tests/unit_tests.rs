...
fn test_mock_client_list_messages() {
    ...
    let messages = result.unwrap();
    let messages_arr = messages.as_array().unwrap();
    assert!(messages_arr.len() > 0, "Should return at least one message with 'meeting'");
    ...
    assert!(subject.to_lowercase().contains("meeting"), "Subject should contain 'meeting'");
}
...
fn test_mock_client_get_message() {
    ...
    assert_eq!(message["id"].as_str().unwrap(), "123", "Message ID should be 123");
    
    // Test getting a non-existent message
    let result = client.get_message("nonexistent");
    assert!(result.is_err(), "Should fail for non-existent message ID");
    assert!(result.unwrap_err().contains("not found"), "Error should indicate message not found");
}
...
fn test_mock_client_list_labels() {
    ...
    assert_eq!(inbox_label["name"].as_str().unwrap(), "Inbox", "INBOX label should be named 'Inbox'");
}
...
fn test_mock_client_get_profile() {
    ...
    assert_eq!(profile["email"].as_str().unwrap(), "test@example.com", "Email should be test@example.com");
    assert!(profile["messagesTotal"].as_u64().is_some(), "Should include messagesTotal");
    assert!(profile["threadsTotal"].as_u64().is_some(), "Should include threadsTotal");
}
...
fn test_mock_client_create_draft() {
    ...
    assert!(draft["message"].is_object(), "Should include message object");
    assert_eq!(
...
fn test_email_conversion() {
    ...
    assert_eq!(json["subject"].as_str().unwrap(), "Test Subject", "Subject should match");
    assert_eq!(json["from"].as_str().unwrap(), "sender@example.com", "From should match");
    assert!(json["bodyText"].as_str().is_some(), "Should include body text");
    assert!(json["bodyHtml"].as_str().is_some(), "Should include HTML body");
}
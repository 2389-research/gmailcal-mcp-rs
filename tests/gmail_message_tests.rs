...
fn encode_base64_url_safe(input: &str) -> String {
    encode(input).replace('+', "-").replace('/', "_")
}
    ...
    match result {
        Err(GmailApiError::MessageFormatError(err)) => {
            assert!(
                ...
                "Error should mention missing threadId field"
            );
        }
...
async fn test_list_messages() {
    ...
    let list_mock = server
        .mock("GET", "/gmail/v1/users/me/messages?maxResults=10")
        .with_status(200)
        .with_header("content-type", "application/json")
    ...
    let msg2_mock = server
        .mock("GET", "/gmail/v1/users/me/messages/msg2?format=full")
        .with_status(200)
        .with_header("content-type", "application/json")
    ...
    let msg3_mock = server
        .mock("GET", "/gmail/v1/users/me/messages/msg3?format=full")
        .with_status(200)
        .with_header("content-type", "application/json")
...
async fn test_list_messages_with_query() {
    ...
    let list_mock = server
        .mock(
            "GET",
            "/gmail/v1/users/me/messages?maxResults=10&q=important",
        )
        .with_status(200)
        .with_header("content-type", "application/json")
        .with_body(messages_json.to_string())
...
async fn test_error_handling_not_found() {
    ...
    let mock = server
        .mock("GET", "/gmail/v1/users/me/messages/nonexistent?format=full")
        .with_status(404)
        .with_header("content-type", "application/json")
...
async fn test_error_handling_network_error() {
    ...
    match result {
        Err(GmailApiError::NetworkError(err)) => {
            ...
            assert!(
                err.contains("error") || err.contains("failed") || err.contains("connection"),

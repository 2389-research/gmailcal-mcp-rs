...
///
use mcp_gmailcal::{
    errors::{ConfigError, GmailApiError},
...
mod server_tests {
    ...
    fn test_command_parsing() {
        ...
        // With no input
        assert_eq!(parse_max_results(None, 10), 10);
    }
    ...
    fn test_error_formatting() {
        ...
        let gmail_errors = [
            (
                GmailApiError::ApiError("API error".to_string()),
                "Gmail API error",
            ),
            (
                GmailApiError::AuthError("Auth error".to_string()),
                "Authentication error",
            ),
            (
                GmailApiError::MessageRetrievalError("Not found".to_string()),
                "Message retrieval error",
            ),
            (
                GmailApiError::MessageFormatError("Invalid format".to_string()),
                "Message format error",
            ),
            (
                GmailApiError::NetworkError("Connection error".to_string()),
                "Network error",
            ),
            (
                GmailApiError::RateLimitError("Too many requests".to_string()),
                "Rate limit error",
            ),
        ];

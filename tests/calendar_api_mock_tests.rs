...
mod tests {
    ...
    fn test_list_calendars_success() {
        ...
        // Setup expectations
        mock.expect_list_calendars().returning(move || {
            Ok(CalendarList {
        ...
        let work_calendar = calendar_list
            .calendars
            .iter()
    ...
    fn test_list_calendars_failure() {
        ...
        mock.expect_list_calendars().returning(|| {
            Err(CalendarApiError::ApiError(
    ...
    fn test_list_events_with_filters() {
        ...
        mock.expect_list_events()
            .with(
                eq("primary"),
                eq(None),
                function(move |t: &Option<DateTime<Utc>>| t.is_some() && t.unwrap() > now_clone),
                eq(None),
            )
    ...
    fn test_list_events_failure() {
        ...
        // Setup expectations for failure
        mock.expect_list_events().returning(|_, _, _, _| {
            Err(CalendarApiError::ApiError(
    ...
    fn test_create_event_success() {
        ...
        // Setup expectations
        mock.expect_create_event().returning(|_, event| {
    ...
    fn test_create_event_validation_failures() {
        ...
        mock.expect_create_event()
            .with(
                function(|c: &str| !c.is_empty()),
                function(|e: &CalendarEvent| e.summary.is_empty()),
            )
            .returning(|_, _| {
                Err(CalendarApiError::EventFormatError(
        ...
        mock.expect_create_event()
            .with(
                function(|c: &str| !c.is_empty()),
                function(|e: &CalendarEvent| !e.summary.is_empty() && e.end_time <= e.start_time),
            )
            .returning(|_, _| {
                Err(CalendarApiError::EventFormatError(
    ...
    fn test_create_event_failure() {
        ...
        mock.expect_create_event().returning(|_, _| {
            Err(CalendarApiError::ApiError(
    ...
    fn test_get_event_not_found() {
        ...
        mock.expect_get_event()
            ...
            .returning(|_, id| {
                Err(CalendarApiError::EventRetrievalError(format!(
                    "Event {} not found",
                    id
                )))
            });

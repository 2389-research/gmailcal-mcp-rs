...
use mcp_gmailcal::people_api::{Contact, EmailAddress, Organization, PersonName, PhoneNumber, Photo, PeopleClient};
...
impl MockPeopleClient {
    ...
    fn parse_contact(&self, data: Value) -> Result<Contact, String> {
        ...
        let resource_name = data.get("resourceName")
            .and_then(|v| v.as_str())
        ...
        if let Some(names) = data.get("names").and_then(|v| v.as_array()) {
            if let Some(name_obj) = names.first() {
                let display_name = name_obj.get("displayName").and_then(|v| v.as_str()).map(|s| s.to_string());
                let given_name = name_obj.get("givenName").and_then(|v| v.as_str()).map(|s| s.to_string());
                let family_name = name_obj.get("familyName").and_then(|v| v.as_str()).map(|s| s.to_string());
        ...
        if let Some(emails) = data.get("emailAddresses").and_then(|v| v.as_array()) {
            for email in emails {
                if let Some(value) = email.get("value").and_then(|v| v.as_str()) {
                    let type_ = email.get("type").and_then(|v| v.as_str()).map(|s| s.to_string());
        ...
        if let Some(phones) = data.get("phoneNumbers").and_then(|v| v.as_array()) {
            for phone in phones {
                if let Some(value) = phone.get("value").and_then(|v| v.as_str()) {
                    let type_ = phone.get("type").and_then(|v| v.as_str()).map(|s| s.to_string());
        ...
        if let Some(orgs) = data.get("organizations").and_then(|v| v.as_array()) {
            for org in orgs {
                let name = org.get("name").and_then(|v| v.as_str()).map(|s| s.to_string());
                let title = org.get("title").and_then(|v| v.as_str()).map(|s| s.to_string());
        ...
        if let Some(pics) = data.get("photos").and_then(|v| v.as_array()) {
            for pic in pics {
                if let Some(url) = pic.get("url").and_then(|v| v.as_str()) {
                    let default = pic.get("default").and_then(|v| v.as_bool()).unwrap_or(false);
    ...
    fn search_contacts(&self, query: &str, _max_results: Option<u32>) -> Result<Vec<Contact>, String> {
        ...
        let filtered_contacts: Vec<Contact> = contacts_data.into_iter()
            .filter(|contact| {
                ...
                display_name.to_lowercase().contains(&query) || email.to_lowercase().contains(&query)
            })
...
mod mock_people_tests {
    ...
    fn test_contact_parsing() {
        ...
        assert_eq!(contact.organizations[0].name.as_ref().unwrap(), "Example Corp");
